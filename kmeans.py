...
def assignment_step(centroids, cluster_atts_idx, data_rows):
    ...
    for datum_idx in range(0, len(data_rows)):
        ...
        closest_centroid_distance, closest_centroid_idx = find_closest_centroid(
            centroids, cluster_atts_idx, datum)
...
def update_centroids(data_rows, cluster_assignments, cluster_atts_idxs, k):
    ...
    for cluster_id in sorted(cluster_assignments):
        ...
        for data_for_cluster_idx in data_for_cluster_idxs:
            ...
            datum_comparable_atts = project_cluster_atts(
                datum, cluster_atts_idxs)
...
def plot_cluster_assignments(cluster_assignments, centroids, data_rows,
    ...
    colors = {0: 'Red', 1: 'Blue', 2: 'Green', 3: 'Purple',
              4: 'Orange', 5: 'Yellow', 6: 'Pink', 7: 'Brown', 8: 'Black', }
    ...
    for idx in range(0, len(plots_configs)):
        ...
        for cluster_assignment in cluster_assignments:
            ...
            dataum_axis_x_data = [cluster_datum[x_raw_data_idx]
                                  for cluster_datum in cluster_data]
            dataum_axis_y_data = [cluster_datum[y_raw_data_idx]
                                  for cluster_datum in cluster_data]
            dataum_axis_x_data, dataum_axis_y_data = sort_for_plot(
                dataum_axis_x_data, dataum_axis_y_data)

            subplot.plot(dataum_axis_x_data, dataum_axis_y_data,
                         marker='o', linestyle='', c=colors[cluster_assignment])
            # centroid
    ...
    results_dir = os.path.join(
        script_dir, 'results/' + plot_config['output_file_prefix'] + '/')

    if 'output_file_prefix' in plot_config:
        fig.savefig(results_dir + (plot_config['output_file_prefix'] +
                    str(image_seq) + ".png"))
        image_seq += 1
    mplpyplot.close(fig)
...
def kmeans(data, k, cluster_atts, cluster_atts_idxs, init_func, plot_config):
    ...
    cluster_assignments, distortion = assignment_step(
        centroids, cluster_atts_idxs, data_rows)
    ...
    while True:
        centroids = update_centroids(
            data_rows, cluster_assignments, cluster_atts_idxs, k)
        next_cluster_assignments, distortion = assignment_step(
            centroids, cluster_atts_idxs, data_rows)
...
def pngs_to_gif(plot_config, k):
    # Create the frames
    frames = []
    resultPath = "./results/" + plot_config['output_file_prefix'] + "/"
    imgs = glob.glob(resultPath + "*.png")
    for i in imgs:
        new_frame = Image.open(i)
        frames.append(new_frame)

    # Save into a GIF file that loops forever
    frames[0].save(resultPath + plot_config['output_file_prefix'] + '_when_k_is_' + str(k) + '.gif', format='GIF',
